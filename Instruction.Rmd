...
```{r My_chunk finding data}
kobe_basket %>% filter(game==1,quarter==1) %>% arrange(time)
...
```{r checking IQR}
kobe_streak %>% summarise(IQR=IQR(length))
#it seams that the shortest is 0 not 1
```
...
```{r finding overall success rate for Kobe}
library(stringr)
num_hit_Kobe <- kobe_basket %>% filter(str_detect(description,"Kobe"),shot=="H") %>% summarise(n())

num_throws_Kobe <- kobe_basket %>% filter(str_detect(description,"Kobe")) %>% summarise(n())

num_hit_Kobe/num_throws_Kobe
```
